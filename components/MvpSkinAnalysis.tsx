...
export const MvpSkinAnalysis: React.FC = () => {
  ...
  const analyzeSkin = async () => {
    ...
    if (!API_KEY) {
      setError("Erro de configuração: Chave da API do Gemini não encontrada.");
      return;
    }
    ...
    try {
      ...
      setError(
        `Erro na análise: ${err.message || "Falha ao conectar com a IA."}`,
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-serif font-bold text-gold-400 flex items-center gap-3">
            ...
            Smart Skin Analysis{" "}
            <span className="text-xs bg-gold-500/20 text-gold-300 px-2 py-0.5 rounded border border-gold-500/30 uppercase tracking-widest font-sans">
              Beta AI
            </span>
          </h2>
            ...
            {image ? (
              ...
              <div className="text-center space-y-4">
                ...
                <p className="text-text-secondary text-sm max-w-xs mx-auto">
                  Clique para tirar uma foto ou selecionar da galeria.
                  Certifique-se de que o rosto esteja bem iluminado.
                </p>
              </div>
            )}
            <input
          ...
          {loading && (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-text-secondary animate-pulse">
                A IA está identificando pontos de melhoria.
              </p>
            </div>
          )}
